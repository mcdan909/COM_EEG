...
function [newEEG] =  EndLocking(eegData)
%eegData = EEG;
...
for trialCounter = 1:length(relevantLatencies)
        currTrialDur = (eegData.event(relevantEventList(trialCounter)).RT...
            + eegData.event(relevantEventList(trialCounter)).MT) * 1000;
        if isempty(currTrialDur)
            currTrialDur = 0;
        end
        revisedRelevantLatencies(trialCounter) = relevantLatencies{trialCounter} + round(currTrialDur/2);
end
