...
for i = 1:length(goodSubs);
    eegData{i} = strcat(goodSubs{i},'_',prefix,'_EEG_WithHiCoh_QuartCurv',suffix,'_Dir.set');
    eegDataCSD{i} = strcat('/CSD/',goodSubs{i},'_',prefix,'_EEGcsd_WithHiCoh_QuartCurv',suffix,'_Dir.set');
end
...
for i = 1:length(goodSubs)
    ...
    fileName = strcat(goodSubs{i},'_',prefix,'_ERP_WithHiCoh_QuartCurv',suffix,'_Dir',filt,'.erp');
    fileNameCSD = strcat('/CSD/',goodSubs{i},'_',prefix,'_ERPcsd_WithHiCoh_QuartCurv',suffix,'_Dir',filt,'.erp');
...
save(strcat('GoodSub_',prefix,'_ERP_WithHiCoh_QuartCurv',suffix,'_Dir',filt,'_FieldTrip'),'erp','erpGrandAve');
save(strcat('GoodSub_',prefix,'_EEG_WithHiCoh_QuartCurv',suffix,'_Dir_FieldTrip'),'eeg');

cd ..

cd CSD/FieldTrip

save(strcat('GoodSub_',prefix,'_ERPcsd_WithHiCoh_QuartCurv',suffix,'_Dir',filt,'_FieldTrip'),'erpCSD','erpGrandAveCSD');
save(strcat('GoodSub_',prefix,'_EEGcsd_WithHiCoh_QuartCurv',suffix,'_Dir_FieldTrip'),'eegCSD');
